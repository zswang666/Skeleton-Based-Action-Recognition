...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        self._rnn_layers = cfg.rnn_layers
        self._rnn_hidden_size = cfg.rnn_hidden_size
        ...
        '''
        for i in range(self._n_primitives):
            fn = nn.Sequential(GGNN(cfg), nn.Linear(self._n_node,1))
            self.add_module("ggnn_{}".format(i), fn)
        self._ggnns = AttrProxy(self, "ggnn_")
        '''
        ...
        self._out = nn.Sequential(
            nn.Linear(self._rnn_hidden_size, self._output_size)
            #nn.Softmax(dim=1)
        )
        '''
    ...
    def forward(self, init_input, data, adj_matrix, seq_length, output_primitives=False):
        ...
        for i in range(self._n_segments):
            ...
            '''
            for i in range(self._n_primitives):
                ggnn_out.append(self._ggnns[i]([init_seg,data_seg,adj_matrix]))
            '''
